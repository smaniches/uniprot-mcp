...
- **2026-04-25** — **Version bumped 0.1.0 → 1.0.1.** Locked-in-step across the four files the drift-prevention contract test pins together: `pyproject.toml` (`version = "1.0.1"`), `.well-known/mcp.json` (`"version": "1.0.1"`), `server.json` (`version_detail.version`, `release_date`, packages[0].version), and `src/uniprot_mcp/client.py` UA string (`uniprot-mcp/1.0.1 (+...)` — the User-Agent every UniProt request now sends, so server-side analytics can identify v1.0.1 traffic separately from any 0.1.0 stragglers). CHANGELOG `[Unreleased]` graduated to `[1.0.1] - 2026-04-25` with a release-summary header. `[Unreleased]` link target now points at v1.0.1...HEAD; v1.0.1 link target reserves the GitHub release URL. **The repo is now wire-ready for the v1.0.1 tag** — the only remaining flip-day actions are: (1) merge `hardening-v2` → `main`, (2) `git tag v1.0.1`, (3) `git push origin v1.0.1` (fires `release.yml` → SLSA + Sigstore + PyPI), (4) `gh repo edit --visibility public` (CodeQL + Scorecard activate on the next push). 287 / 287 tests, contract test `test_pyproject_version_matches_well_known` confirms lock-step.
