...
as [`uniprot-mcp-server` v1.1.7](https://pypi.org/project/uniprot-mcp-server/1.1.7/).
...
| 744 offline + 42 live integration tests (real counts via `pytest --collect-only` at v1.1.6). | `pytest --ignore=tests/integration -q` for the offline 744; `pytest --integration tests/integration` for the 42. |
...
| The PyPI wheel was built from this exact repo. | SLSA build provenance attestation on every [GitHub Release](https://github.com/smaniches/uniprot-mcp/releases) (v1.1.7 = latest); `gh attestation verify` confirms. End-to-end script: `scripts/replicate.sh`. |
...
1. Downloads `uniprot-mcp-server==1.1.7` from PyPI (override with the `VERSION` env var to pin any released version); SHA-256s it.
